...
const shellCategories: Array<{ category: string; contract: ContentCategory; image: string; note: string }> = [
  ...
  { category: "展览", contract: "exhibition", image: "/art/yike/lavender-exhibition.png", note: "把灵感带回花园" },
...
const achievements: Achievement[] = [
  { id: "first-heart", category: "record", title: "第一株心动", body: "第一次留下属于你和花园的温柔记忆", image: "/art/yike/achievement-first-heart.png", metric: "completed", target: 1 },
...
const feedbackActionText: Record<FeedbackAction, string> = { accept: "好，就它了", complete: "已完成", reroll: "换一张", not_suitable: "当下不合适", later: "以后再说", dislike: "不喜欢" };
  ...
  return <div className="view home-view">
    ...
    <section className="hero-card journal-hero journal-reference-hero">
      ...
      <div className="journal-dynamic-copy">
        ...
        <p>白天种下的心动，留到夜里慢慢拾起。<br />来和小宜说说吧，我们一起从收藏夹里，找回一件刚刚好的小欢喜。</p>
      </div>
    </section>
    ...
    <section className="pick-section"><div className="section-heading"><div><span className="section-index">01</span><h2>今晚，拾起一件小欢喜，捧起一束薰衣草</h2></div><p>小宜会照顾你的时间、精力和去处，也悄悄留一点点惊喜。</p></div>
      <div className="source-selector"><span>今晚，想让小宜从哪里为你挑选？</span><div>{(["personal", "preset", "both"] as SourceScope[]).map((source) => <Chip key={source} active={context.source === source} onClick={() => setContext((value) => ({ ...value, source }))}>{sourceText[source]}</Chip>)}</div></div>
      <button className={`card-pack draw-ritual phase-${drawPhase}`} type="button" onClick={onDraw} disabled={drawing} aria-label={drawing ? "正在抽取一张娱乐卡" : "朝花夕拾"}>
        <span className="pack-stitch" />
        <span className="draw-copy"><span className="pack-label">TONIGHT&apos;S PICK</span>{drawing ? <strong>小宜正在唤醒花种…</strong> : <img className="draw-handwritten" src="/art/yike/handwritten-draw.png" alt="轻轻打开" />}<small>{contextSummary}</small></span>
      ...
      <button className="primary-button draw-button" type="button" onClick={onDraw} disabled={drawing}>{drawing ? "正在匹配此刻…" : "朝花夕拾"}</button>
    </section>
    ...
    {personalCount === 0 && <div className="cold-start"><div className="mini-shell">◇</div><div><strong>花园里还空空的</strong><p>先种下第一株薰衣草，今晚就能收获啦。</p></div><button type="button" onClick={onAdd}>种下一株薰衣草</button></div>}
  </div>;
}
...
function PageRail({ view, cardCount, context, contextSummary, ambient, petalBalance, petalLedger, onRefreshAmbient, setContext, onNavigate }: {
  ...
  if (view === "pool") {
    return <aside className="desktop-context page-rail pool-rail"><div className="context-title"><span>花圃小记</span><b>ATLAS</b></div><img className="rail-shell" src="/art/yike/lavender-movie.png" alt="薰衣草" /><h2>{cardCount} 张卡、九种薰衣草</h2><p>每一簇薰衣草，都盛放着一类心动。选中它，就能把收藏里的小欢喜，轻轻拾回来。</p><div className="rail-note"><strong>图鉴规则</strong><span>有收藏的分类会显示数量；暂时空着的也会保留位置，等你慢慢收进喜欢的花簇。</span></div></aside>;
  }
  if (view === "add") {
    return <aside className="desktop-context page-rail capture-rail"><div className="context-title"><span>先交给小宜</span><b>3 STEPS</b></div><ol className="capture-steps"><li><b>1</b><div><strong>先认一认</strong><span>找出标题、类别和内容线索</span></div></li><li><b>2</b><div><strong>再补一补</strong><span>估一估需要的时间、力气和准备</span></div></li><li><b>3</b><div><strong>你点头后再收好</strong><span>确认合适，再放进你的心动收藏夹</span></div></li></ol><img className="rail-otter capture-heart-otter" src="/art/yike/achievement-first-heart.png" alt="抱着薰衣草盆的小宜" /><div className="rail-note privacy"><strong>图片仅用于本次整理</strong><span>原图默认私有，不会公开展示。</span></div></aside>;
  }
  if (view === "achievements") {
    return <AchievementRail petalBalance={petalBalance} petalLedger={petalLedger} onNavigate={onNavigate} />;
  }
  return <aside className="desktop-context page-rail memory-rail"><div className="context-title"><span>只陪你过今晚的事</span><b>PRIVATE</b></div><img className="rail-shell memory-care-otter" src="/art/yike/otter-period-care.png" alt="抱着热水袋的小宜" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = "/art/yike/achievement-first-heart.png"; }} /><h2>有些小状况，小宜只在今晚记得</h2><ul><li>经期不舒服</li><li>不想久站</li><li>不想费心打扮</li></ul><p>这些只用来照顾当下，不会被写成长期偏好，也不会被拿来猜测你的身体或性格。</p><div className="rail-note privacy"><strong>这些记忆都由你做主</strong><span>看得到、改得了、删得掉，也可以全部清空。</span></div></aside>;
}
...
function AddView({ inputText, imageName, imagePreview, parseStep, draft, isEditing, savingDraft, fileInputRef, setInputText, setDraft, onImage, onParse, onSave }: {
  ...
  return <div className="view add-view"><div className="eyebrow">{isEditing ? "EDIT · 修改卡片" : "CAPTURE · 种下一株薰衣草"}</div><div className="page-title"><div><img className="page-title-handwritten" src="/art/yike/handwritten-add.png" alt="把收藏夹里的心动，变成今晚能拾起的一件事" /><p>{isEditing ? "修改字段后点击保存，不会新增重复卡片。" : "截图、照片或一句话都可以。小宜会先帮你认出来，再陪你补好适合什么时候做。"}</p></div><span className="step-badge">{isEditing ? "编辑中" : "约 10 秒"}</span></div>
...
function PoolView({ cards, onAdd, onArchive, onDelete, onEdit }: { cards: Card[]; onAdd: () => void; onArchive: (id: string) => void; onDelete: (id: string) => void; onEdit: (card: Card) => void }) {
  ...
  return <div className="view pool-view"><div className="eyebrow">COLLECTION · 薰衣草花圃</div><div className="page-title"><div><img className="page-title-handwritten" src="/art/yike/handwritten-pool.png" alt="收进来的芬芳" /><p>每一簇薰衣草，都替你留着一份心动。轻轻点开，把白天收藏的念想，在今晚慢慢拾起。</p></div><button className="primary-button compact" type="button" onClick={onAdd}>种下一株薰衣草</button></div>
    <section className="shell-atlas" aria-labelledby="shell-atlas-title"><div className="atlas-heading"><div><span>薰衣草图鉴</span><h2 id="shell-atlas-title">挑一簇心动的薰衣草，开始逛逛吧</h2></div><button type="button" className={selectedCategory ? "" : "active"} onClick={() => setSelectedCategory(null)}>查看全部</button></div><div className="shell-atlas-grid">{shellCategories.map((item) => { const count = counts[item.category] ?? 0; const selected = selectedCategory === item.category; return <button type="button" key={item.category} className={`${selected ? "selected" : ""} ${count === 0 ? "empty" : ""}`} aria-pressed={selected} onClick={() => setSelectedCategory(selected ? null : item.category)}><img src={item.image} alt={`${item.category}类别薰衣草`} /><strong>{item.category}</strong><span>{count ? `${count} 张卡` : "等第一份心动，在这里慢慢开花"}</span></button>; })}</div></section>
    <div className="pool-toolbar"><div className="search-box">⌕<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="找找某部剧、某家店，或那首歌" /></div><div className="pool-count"><strong>{cards.length}</strong><span>颗已收藏</span></div></div>
...
function ResultView({ card, reasons, revealing, feedbackOpen, feedbackSubmitting, feedbackInsight, acceptanceNote, careNotice, ambient, setFeedbackOpen, onAccept, onExchange, onContext, onFeedback, onCopy }: {
  ...
  return <div className={`view result-view ${revealing ? "is-revealing" : ""}`}>
    ...
    <div className="result-heading"><div><h1>今晚，就从这个开始</h1><p>小宜只为你挑这一件，也会轻轻告诉你为什么是它。</p></div><img src="/otter-side.png" alt="为你揭晓结果的海獭小宜" /></div>
    ...
    <div className="result-actions"><button className="primary-button" type="button" onClick={onAccept} disabled={feedbackSubmitting || revealing || Boolean(acceptanceNote)}>{feedbackSubmitting ? "记录中…" : acceptanceNote ? "已选择" : "好，就它了"}</button>{acceptanceNote && <div className="acceptance-note">{acceptanceNote}</div>}<div><button className="secondary-button" type="button" onClick={onExchange} disabled={feedbackSubmitting || revealing}>换一张</button><button className="secondary-button" type="button" onClick={onContext} disabled={feedbackSubmitting || revealing}>改条件</button></div>{card.source === "preset" && <button className="text-button" type="button" onClick={onCopy} disabled={feedbackSubmitting || revealing}>存成我的卡</button>}{!acceptanceNote && <button className="feedback-link" type="button" onClick={() => setFeedbackOpen(!feedbackOpen)} disabled={feedbackSubmitting || revealing}>{feedbackOpen ? "收起反馈" : "这张卡怎么样？"}</button>}</div>
...
function ActivityView({ activity, completionOpen, feedbackSubmitting, petalBalance, ambient, onOpenCompletion, onCloseCompletion, onComplete, onBackHome, onMemory }: {
  ...
  return <div className="view activity-view">
    ...
    <div className="result-heading"><div><h1>{completed ? "这件事已经完成啦" : timeUp ? "预计时间到啦" : "这件事正在进行中"}</h1><p>{completed ? "完成记录已经留下，花瓣也收进记忆里。" : timeUp ? "可以结束并记录这次体验，也可以再多享受一会儿。" : "小宜会先陪你收获这一束薰衣草，结束前不会来打扰你。"}</p></div><img src="/otter-side.png" alt="陪你开始活动的海獭小宜" /></div>
    <article className={`activity-card ${completed ? "completed" : ""}`}>
      ...
      <div className="activity-content"><div className="result-badges"><SourceBadge source={card.source} /><span>{card.category}</span></div><h2>{card.title}</h2><p className="result-meta">预计 {card.duration} 分钟　·　{card.outing === "indoor" ? "室内" : card.outing === "outdoor" ? "室外" : "均可"}　·　{levelText[card.prep]}准备</p>
        ...
        {!completed && <div className={`activity-countdown ${timeUp ? "time-up" : ""}`}><div><span>{timeUp ? "预计时间已到" : "剩余时间"}</span><strong>{timeUp ? "可以结束记录" : formatRemaining(remainingMs)}</strong></div><div className="activity-progress" aria-hidden="true"><i style={{ width: `${progress}%` }} /></div><p>{timeUp ? "完成确认会帮助小宜记录反馈，并把花瓣放进花瓣罐。" : "进行时，小宜会先替你收起新的选择，让今晚只专心耕耘这一片花丛。"}</p></div>}
        {card.source === "preset" && <div className="soft-benefit"><span>小宜送你的花瓣</span><p>这张推荐卡保留一个轻量权益占位，完成时先用页面确认。</p><strong>YIKE-{card.category.toUpperCase().slice(0, 2)}-{String(activity.rewardPetals).padStart(2, "0")}</strong></div>}
...
function softenPreferenceEvidence(evidence: string) {
  return evidence
    .replace(/^来自\s*/, "这是小宜从你 ")
    .replace(/次主动选择或完成记录$/, "次主动选择或完成里，慢慢记住的。");
}
...
function PetalPool({ balance, ledger }: { balance: number; ledger: PetalLedgerEntry[] }) {
  ...
  const plotStage = gardenStageFor(balance, 0);
  return <aside className="petal-pool garden-pool" aria-label="薰衣草花瓣罐"><div className="garden-pool-head"><div><span>薰衣草花瓣罐</span><strong>{balance}</strong><small>片薰衣草花瓣</small></div><em>{growthUnits}/12 生长进度</em></div><div className="lavender-garden single-plot"><article className={`garden-plot ${plotStage}`}><img src={`/art/yike/garden-${plotStage}-1.png`} alt="" /><span>1</span><small>{gardenStageText[plotStage]}</small></article></div><p>{growthUnits >= 12 ? "这块土地已经开花。后续可以接真实账本，把花瓣兑换和联名礼品放进这里。" : `每 2 片花瓣推进一格生长，再收集 ${Math.max(1, nextPetals)} 片就会出现新的变化。`}</p><div className="petal-ledger-preview">{recent.length === 0 ? <span>完成一次活动后，这里会出现第一条花瓣记录。</span> : recent.map((entry) => <article key={entry.id}><b>+{entry.amount}</b><div><strong>{entry.cardTitle}</strong><small>{entry.reason}</small></div></article>)}</div></aside>;
}
...
function MemoryView({ memoryNote, memorySummary, feedbackInsight, debugLog, petalBalance, petalLedger, onLoadHistory, onMemoryAction, onReset }: {
  ...
  const monthlyHarvestCount = summaryCalendar?.completed_count ?? 0;
  const monthlySeedCount = summaryCalendar ? Math.min(summaryCalendar.positive_count, monthlyHarvestCount) : 0;
  ...
  return <div className="view memory-view"><div className="eyebrow">MEMORY · 由你做主</div><div className="page-title"><div><img className="page-title-handwritten" src="/art/yike/handwritten-memory.png" alt="小宜记得什么" /><p>小宜只记住能让下一次更合适的小偏好。你随时可以查看、改一改，或让它忘掉。</p></div></div>
    ...
    <section className="memory-calendar"><img className="calendar-shell-frame" src="/art/yike/calendar-stamp-lavender.png" alt="" /><div className="calendar-content"><div className="calendar-head"><button type="button" onClick={() => moveMonth(-1)} aria-label="上个月">←</button><div><span>拾花日历</span><h2>{month.getFullYear()} 年 {month.getMonth() + 1} 月</h2></div><button type="button" onClick={() => moveMonth(1)} aria-label="下个月">→</button></div><div className="calendar-weekdays" aria-hidden="true">{["一", "二", "三", "四", "五", "六", "日"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{days.map((date) => { const key = localDateKey(date); const dayEvents = eventsByDate[key] ?? []; const outside = date.getMonth() !== month.getMonth(); return <button type="button" key={key} className={`${outside ? "outside" : ""} ${selectedDate === key ? "selected" : ""}`} onClick={() => setSelectedDate(key)} aria-label={`${date.getMonth() + 1}月${date.getDate()}日，${dayEvents.length}条记录`}><span>{date.getDate()}</span><i>{dayEvents.slice(0, 3).map((event) => <b key={event.event_id} className={event.kind} />)}</i></button>; })}</div></div></section>
    <PetalPool balance={petalBalance} ledger={petalLedger} />
    </div>
    <section className="day-memory"><div className="day-memory-title"><div><span>{selectedDate}</span><h2>这一天收获的薰衣草</h2></div>{events.some((event) => event.is_demo) && <b>演示记录</b>}</div>{historyLoading ? <p className="calendar-message">正在从花园里读取记录…</p> : historyError ? <div className="calendar-message error"><p>暂时无法读取：{historyError}</p><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重试</button></div> : selectedEvents.length === 0 ? <p className="calendar-message">这天花园很安静，没有留下新的记录。</p> : <div className="day-event-list">{selectedEvents.map((event) => { const category = categoryFromContract[event.content_category]; const shell = shellForCategory(category); return <article key={event.event_id}><img src={shell.image} alt="" /><div><span>{event.kind === "draw" ? "小宜挑中" : feedbackActionText[event.action ?? "accept"]}</span><strong>{event.title}</strong><small>{new Date(event.occurred_at).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })} · {category}</small></div></article>; })}</div>}</section>
    <div className="memory-grid"><section className="memory-card blue"><span>这个月的小小回响</span><h2>{summaryCalendar ? `这个月收获了 ${summaryCalendar.pearl_count} 束薰衣草` : memoryNote}</h2><p>{summaryCalendar ? `${summaryCalendar.month_label} · 丰收 ${monthlyHarvestCount} 次，播种 ${monthlySeedCount} 次。` : feedbackInsight ? `${feedbackInsight.shortTerm}；${feedbackInsight.cooldown}。` : "你可以随时撤回，系统不会据此建立人格或健康标签。"}</p></section><section className="memory-card"><span>长期偏好</span><h2>{preference?.headline ?? (feedbackInsight ? feedbackInsight.memoryShift : "室内 · 低准备 · 45 分钟")}</h2><div className="memory-tags">{(preference?.tags ?? [{ label: "活动场景", value: "室内" }, { label: "准备程度", value: "低" }, { label: "可用时长", value: "45 分钟" }]).map((tag) => <em key={`${tag.label}-${tag.value}`}>{tag.label}：{tag.value}</em>)}</div><p>{preference?.evidence ? softenPreferenceEvidence(preference.evidence) : (feedbackInsight ? feedbackInsight.longTerm : "这里只展示用户主动选择和可解释的行为信号。")}</p></section></div>
